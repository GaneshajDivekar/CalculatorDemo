...
class CalculatorImpl (
    ...
    private var inputDisplayedFormula = "0.00"
    private var lastKey = ""
        ...
        if (!value.contains(".")) {
            when {
                value == "0" && !valueToCheck.contains(operationsRegex.toRegex()) -> inputDisplayedFormula =
                    "0.00"
                value == "" -> inputDisplayedFormula += "0.00"
                else -> inputDisplayedFormula += ".00"
            }
        }
    ...
    private fun showNewResult(value: String) {
        ...
        if (checkDecimal == true) {
            ...
            if (value.contains(".")) {
                try {
                    val separated: Array<String> = value.split(".").toTypedArray()
                    if (separated[1].length > 2) {
                        updatedValue = separated[0] + "." + separated[1].substring(
                            separated[1].length - 2,
                            separated[1].length
                        )
                    } else {
                        updatedValue = value
            ...
            inputDisplayedFormula = updatedValue
            callback!!.showNewResult(updatedValue, context)
        } else {
            if (!value.contains("."))
                callback!!.showNewResult(value+".00", context)
            else
    ...
    fun handleClear() {
        try {
            var newValue: String = ""
            val separated: Array<String> = inputDisplayedFormula.split(".").toTypedArray()
            if (separated[1] == "00" || separated[1] == "0" || separated[1] == "") {
                newValue = separated[0].dropLast(1)
            } else {
                newValue = inputDisplayedFormula.dropLast(1)
            }
            if (newValue.equals("")) {
                newValue = "0"
            }
            ...
            if (newValue.contains(".")) {
                var updatedValue = ""
                val separated: Array<String> = newValue.split(".").toTypedArray()
                if (separated[1].length > 2) {
                    updatedValue = separated[0] + "." + separated[1].substring(
                        separated[1].length - 2,
                        separated[1].length
                    )
                    if (separated[1].isNullOrEmpty()) {
                        val updatedZeros = newValue + ".00"
                        showNewResult(updatedZeros)
                    } else if (separated[1].length < 2) {
                        val updatedZeros = newValue + ".00"
                        showNewResult(updatedZeros)
                    }
                } else {
